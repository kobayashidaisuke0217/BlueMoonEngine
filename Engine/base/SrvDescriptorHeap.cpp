...
const uint32_t SrvDescriptorHeap::MAX_SRV_COUNT_ = 512;
...
void SrvDescriptorHeap::Initialize() {
	//デスクリプタヒープの生成
	SrvDescriptorHeap::GetInstance()->descriptorHeap_ = DirectXCommon::GetInstance()->GenarateDescriptorHeap(
		D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, MAX_SRV_COUNT_, true);

	//SRV...ShaderResourceView
	//デスクリプタ1個分のサイズを取得して記録
	SrvDescriptorHeap::GetInstance()->descriptorSize_ = DirectXCommon::GetInstance()->GetDevice()->
		GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);


}

uint32_t SrvDescriptorHeap::Allocate() {
...
D3D12_CPU_DESCRIPTOR_HANDLE SrvDescriptorHeap::GetCPUDescriptorHandle(uint32_t index) {
	D3D12_CPU_DESCRIPTOR_HANDLE handleCPU = descriptorHeap_->GetCPUDescriptorHandleForHeapStart();
	handleCPU.ptr += (descriptorSize_ * index);
	return handleCPU;
}

D3D12_GPU_DESCRIPTOR_HANDLE SrvDescriptorHeap::GetGPUDescriptorHandle(uint32_t index) {
...
void SrvDescriptorHeap::CreateSRVForTexture2D(uint32_t srvIndex, ID3D12Resource* pResource, DXGI_FORMAT format, UINT mipLevels) {
...
void SrvDescriptorHeap::PreDraw() {
	////コマンドを積む
...
void SrvDescriptorHeap::SetGraphicsRootDescriptorTable(UINT rootParameterIndex, uint32_t srvIndex) {
